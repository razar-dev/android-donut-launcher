...
public class LiveFolder extends Folder {
    ...
    private static boolean isDisplayModeList(FolderInfo folderInfo) {
        return ((LiveFolderInfo) folderInfo).getDisplayMode() ==
                LiveFolders.DISPLAY_MODE_LIST;
    }

    @Override
    public void onItemClick(@NotNull AdapterView parent, View v, int position, long id) {
        LiveFolderAdapter.ViewHolder holder = (LiveFolderAdapter.ViewHolder) v.getTag();

        if (holder.getUseBaseIntent()) {
            final Intent baseIntent = ((LiveFolderInfo) getInfo()).getBaseIntent();
            if (baseIntent != null) {
                final Intent intent = new Intent(baseIntent);
                Uri uri = baseIntent.getData();
                uri = uri.buildUpon().appendPath(Long.toString(holder.getId())).build();
                intent.setData(uri);
                // set bound
                Rect targetRect = new Rect();
                v.getGlobalVisibleRect(targetRect);
                intent.setSourceBounds(targetRect);
                getMLauncher().startActivitySafely(intent);
            }
        } else if (holder.getIntent() != null) {
            getMLauncher().startActivitySafely(holder.getIntent());
        }
    }
    ...
    public void bind(@NotNull FolderInfo info) {
        super.bind(info);
    ...
    public void onClose() {
        ...
        final LiveFolderAdapter adapter = (LiveFolderAdapter) getMContent().getAdapter();
        if (adapter != null) {
            adapter.cleanup();
        }
    }
    ...
    static class FolderLoadingTask extends AsyncTask<LiveFolderInfo, Void, Cursor> {
        ...
        protected Cursor doInBackground(LiveFolderInfo... params) {
            ...
            if (folder != null) {
                mInfo = params[0];
                return LiveFolderAdapter.Companion.query(folder.getMLauncher(), mInfo);
            }
            return null;
        }
        ...
        protected void onPostExecute(Cursor cursor) {
            if (!isCancelled()) {
                if (cursor != null) {
                    final LiveFolder folder = mFolder.get();
                    if (folder != null) {
                        final Launcher launcher = folder.getMLauncher();
